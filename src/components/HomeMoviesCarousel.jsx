                            ...
                            <div data-aos='fade-up' data-aos-delay={`${(index + 1) * 100}`} className='movies-slider-container'>
                                <div className='movies-slider'>
                                    <div className='movies-slider-icons'>
                                        <button onClick={() => { Router.push(`/content/${movie.id}?name=${movie.name !== undefined ? movie.name : movie.title}&type=${movie.media_type !== undefined ? movie.media_type : movie.original_title !== undefined ? 'movie' : movie.name !== undefined ? 'tv' : 'movie'}&play=true`); sendFilteredContent({ filteredMovies: movie }) }} className='play' />
                                        <button onClick={() => { Router.push(`/content/${movie.id}?name=${movie.name !== undefined ? movie.name : movie.title}&type=${movie.media_type !== undefined ? movie.media_type : movie.original_title !== undefined ? 'movie' : movie.name !== undefined ? 'tv' : 'movie'}`); sendFilteredContent({ filteredMovies: movie }) }} className='info' />
                                    </div>
                                    <img onClick={() => { Router.push(`/content/${movie.id}?name=${movie.name !== undefined ? movie.name : movie.title}&type=${movie.media_type !== undefined ? movie.media_type : movie.original_title !== undefined ? 'movie' : movie.name !== undefined ? 'tv' : 'movie'}`); sendFilteredContent({ filteredMovies: movie }) }} src={`https://image.tmdb.org/t/p/w500/${movie.poster_path}`} alt={movie.name !== undefined ? movie.name : movie.title} width={180} height={270} />
                                </div>
                            </div>
                        </SwiperSlide>
                        :
